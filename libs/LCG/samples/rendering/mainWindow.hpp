...
class MainWindow {
	...
	public:
		...
		**/
		 ...
		 * @return Returns a pointer to the mesh object.
		**/
		Mesh* getMesh() {
			return mesh;
		}
	...
	private:
		...
		**/
		...
		///Creates the Screen Space Ambient Occlusion Effect.
		void createSSAO();

		///Creates the Toon Effect.
		void createToon();

		///Creates the Compute Shader Test Effect.
		void createCompute();
		...
		/** Vector storing all the effects pointers. Effects are used when some kind of pre-computation is needed for using a shader, i.e., there will be a source file that will do any kind of computations
		 *  needed before the shader is activated and the mesh is rendered. When an effect is active, the rendering will be done directly from the effect's object render function.
		**/
