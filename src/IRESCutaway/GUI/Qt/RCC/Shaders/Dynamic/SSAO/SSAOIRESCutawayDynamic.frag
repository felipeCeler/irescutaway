...
//                if ( color_t.a < 2.0)
        ...
        else if ( color_t.a == 1.0 )
        {
                I = 0.0;
                // make sure we are at the center of the pixel to make the right texture access
                pixel_pos = vec2 ( floor ( gl_FragCoord.x ) , floor ( gl_FragCoord.y ) ) + vec2 ( 0.5 );
                // cutaway normal = rgb, and cutaway depth in camera space = w
                cutaway = texelFetch ( normal , ivec2 ( pixel_pos ) , 0 ).rgba;

                vec2 dist_neighbor[8] =
                { vec2 ( linesize , 0 ), vec2 ( -linesize , 0 ), vec2 ( 0 , linesize ), vec2 ( 0 , -linesize ), vec2 ( -linesizediag , -linesizediag ), vec2 ( -linesizediag , linesizediag ), vec2 ( linesize , -linesize ), vec2 (
                                                                                                                                                linesizediag , -linesizediag ) };
        ...
        else
        {
                ...
                newNormal = normalize(VertexIn.normal.xyz);
                newVert = VertexIn.vertice.xyz;
...
//                        }
                ...
                out_Coords = vec4 ( VertexIn.vertice.xyz , 1.0 );
                out_Normal = vec4 ( VertexIn.normal.xyz , 1.0 );
